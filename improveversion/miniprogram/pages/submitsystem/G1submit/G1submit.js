const app = getApp()
const DB = wx.cloud.database().collection("submit")
let score1 = 0
let score2 = 0
...
Page({
    ...
    db.collection('submit').where({
      ...
      success: res => {
        ...
        if(res.data[0].issubmitted){//已经提交则从数据库中获取数据至页面
          this.setData({
            boolfirst : true,
            G11Score : res.data[0].G11Score,
            G12Score : res.data[0].G12Score
          })
        }
        else{
  ...
  stepbackward(){
    wx.redirectTo({
      url: "/pages/submitsystem/G21submit/G21submit"
    })
  },
  // 删除照片 
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.cancel) {
          ...
          if(score1 > 70 || score1 < 0 || score2 > 30 || score2 < 0){
            wx.showToast({
              title: '请输入合理的成绩',
              icon: 'none',
              duration: 2000
            ...
            if (!that.data.imgbox.length) {
              ...
              for (let i = 0; i < that.data.imgbox.length; i++) {
                promiseArr.push(new Promise((reslove, reject) => {
                  ...
                  str = num.toString()
                  wx.cloud.uploadFile({
                    cloudPath: 'G1/' + new Date().getTime() + str , // 上传至云端的路径
              ...
              Promise.all(promiseArr).then(res => {//等数组都做完后做then方法，开始上传成绩
                ...
                DB.add({//成绩上传至数据库
                  data:{
                    G11Score : parseFloat(score1),
                    G12Score : parseFloat(score2),
                    issubmitted:true,
                    ispermitted:false,
                    _fileIDs: temp,
                    checker1 : '',

const app = getApp()
const DB = wx.cloud.database().collection("submit3")
let score1 = 0
let score2 = 0
...
Page({
    ...
    db.collection('submit3').where({
      ...
      success: res => {
        ...
        if(res.data[0].issubmitted){//已经提交则从数据库中获取数据至页面
          this.setData({
            boolfirst : true,
            G31Score : res.data[0].G31Score,
            G32Score : res.data[0].G32Score
          })
        }
        else{
  ...
  stepbackward(){
    wx.redirectTo({
      url: "/pages/submitsystem/G41submit/G41submit"
    })
  },
  // 删除照片 
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.cancel) {
          ...
          if(score1 > 50 || score1 < 0 || score2 > 50 || score2 < 0){
            wx.showToast({
              title: '请输入合理的成绩',
              icon: 'none',
              duration: 2000
            ...
            if (!that.data.imgbox.length) {
              ...
              for (let i = 0; i < that.data.imgbox.length; i++) {
                promiseArr.push(new Promise((reslove, reject) => {
                  ...
                  str = num.toString()
                  wx.cloud.uploadFile({
                    cloudPath: 'G3/' + new Date().getTime() + str , // 上传至云端的路径
              ...
              Promise.all(promiseArr).then(res => {//等数组都做完后做then方法，开始上传成绩
                ...
                DB.add({//成绩上传至数据库
                  data:{
                    G31Score : parseFloat(score1),
                    G32Score : parseFloat(score2),
                    issubmitted:true,
                    ispermitted:false,
                    _fileIDs: temp
                  }, 
